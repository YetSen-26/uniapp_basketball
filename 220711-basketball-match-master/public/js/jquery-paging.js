...
(function($) {
	...
	$.fn.extend({
		"paging": function(options) {
			...
			This.hide();
			first();
			var numLI = Math.ceil((opts.total) / opts.PageNum);
			num = 0
				...
				if (index == 1) {
					if (btnActive.index() == 0) {
						...
						if (prev[prevLen] == numLI) {
							lt = opts.PageNum * numLI, gt = ((opts.PageNum * numLI) - opts
								.PageNum);
						}
						var arrHasLen = 0
						...
						if (arrHas[arrHasLen] == $(".paging_content .paging_textBox button")
							.html()) {
				...
				if (opts.pagingBtnPaging == true) {
					if (spanHtml > numLI - opts.PageBtn + 1) {
						...
						if (spanHtml == 1) {
							...
							$(".paging_btn span:eq(" + (spanHtml + (opts.PageBtn - 2)) + ")").prevAll()
								.show();
			...
			function prevPaging(spanHtml) {
				if (opts.pagingBtnPaging == true) {
					if (spanHtml < 3) {
						...
						$(".paging_btn span:eq(" + (spanHtml - (opts.PageBtn - 2)) + ")").nextAll()
							.show();
			...
			if (opts.pageInput == true) {
				// $(".paging_content").html("");
				$(".paging_content").append(
					"<div class='paging_textBox'>第<input type='text'/>页<button>前往</button></div>");
			...
			function main(mainNumLt, mainNumGt) {
				This.hide()
				$(".paging_box:lt(" + mainNumLt + "):gt(" + mainNumGt + ")").css("display", opts
					.pagingDisplay);
			}
